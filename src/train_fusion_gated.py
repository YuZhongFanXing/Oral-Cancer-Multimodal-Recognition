"""
Img_SE + Fusion_Gated 组合实验 -- 5折交叉验证
===============================================
在 Img_SE 实验基础上，将融合方式从 Concat 替换为 GatedFusion。
GatedFusion 与架构实验脚本中 Fusion_Gated 完全一致。

ArchModel 对应路径:
  1. backbone    = get_efficientnet_v2s()               img_out=1280
  2. img_se      = SEBlock(1280, reduction=16)           ← Img_SE
  3. meta_branch = MetaMLP_Baseline(5, out_dim=256)      meta_out=256
  4. fusion      = GatedFusion(1280, 256, proj_dim=512)  ← Fusion_Gated, fusion_dim=512
  5. classifier  = build_head_baseline(512)              512->1024->512->256->2

forward:
  img_feat  = backbone(img)                 # (B, 1280)
  img_feat  = img_se(img_feat)              # (B, 1280)  SE通道重标定
  meta_feat = meta_branch(meta)             # (B, 256)
  fused     = GatedFusion(img_feat,meta_feat)  # (B, 512)
...
class Config:
    ...
    N_FOLDS   = 5
    TEST_SIZE = 0.15
    SAVE_ROOT = os.path.join(BASE_DIR, "results_img_se_gated_cv5_seed42")
...
class GatedFusion(nn.Module):
    """
    可学习gate: g = sigmoid(W[img;meta]) 控制两者的贡献比例.
    img和meta先投影到同一维度再加权融合.
    """
    ...
    def forward(self, img_feat, meta_feat):
        img_p  = F.silu(self.img_proj(img_feat))
        meta_p = F.silu(self.meta_proj(meta_feat))
        g      = torch.sigmoid(self.gate(torch.cat([img_p, meta_p], dim=1)))
        return g * img_p + (1 - g) * meta_p
...
# 7. 模型类
# ==============================================================================
class ImgSEGatedModel(nn.Module):
    """
    初始化顺序:
      1. backbone    = get_efficientnet_v2s()               img_out=1280
      2. img_se      = SEBlock(1280, reduction=16)
      3. meta_branch = MetaMLP_Baseline(meta_dim, 256)      meta_out=256
      4. fusion      = GatedFusion(1280, 256, proj_dim=512) fusion_dim=512
      5. classifier  = build_head_baseline(512)

    与 Fusion_Gated 实验的唯一区别:
      img_se = SEBlock(...)  而非  img_se = None
      图像特征在进入GatedFusion之前先经过SE通道重标定。
    """
    def __init__(self, meta_dim=5, num_classes=2):
        super().__init__()
        # step 1: backbone
        self.backbone, img_out = get_efficientnet_v2s()           # 1280d
        # step 2: SE on image features
        self.img_se = SEBlock(img_out, reduction=16)               # SEBlock(1280,16)
        # step 3: meta branch
        self.meta_branch = MetaMLP_Baseline(meta_dim, out_dim=256)
        meta_out  = 256
        # step 4: GatedFusion (与架构实验脚本 fusion_gated 分支完全一致)
        self.fusion   = GatedFusion(img_out, meta_out, proj_dim=512)
...
def plot_cv_summary(all_results, save_dir):
    ...
    plt.suptitle(
        ...
        f'EfficientNetV2-S + SE(img) + GatedFusion | Fixed test set | seed=42',
        fontsize=11, fontweight='bold')
    plt.tight_layout()
...
def main():
    ...
    print(f"  Fusion   : GatedFusion(img_dim=1280, meta_dim=256, proj_dim=512)")
    print(f"  Head     : FC(512->1024->512->256->2)")
    print(f"  Loss     : WLS(w=[2,1], s=0.05)")
    print(f"{'='*65}")
    print(f"\n  与各单独实验的关系:")
    print(f"    Img_SE 实验    : img_se=SEBlock  fusion=Concat(1536d)")
    print(f"    Fusion_Gated实验: img_se=None    fusion=GatedFusion(512d)")
    print(f"    本实验          : img_se=SEBlock  fusion=GatedFusion(512d)  ← 两者结合")
    ...
    lines = [
        ...
        "  Img SE     : SEBlock(dim=1280, reduction=16)  维度不变",
        "               x_out = x * sigmoid(FC(ReLU(FC(x))))",
        "  Meta branch: MetaMLP_Baseline(5->128->256)  ->  256d",
        "  Fusion     : GatedFusion(img_dim=1280, meta_dim_out=256, proj_dim=512)",
        "               g = sigmoid(W[SiLU(img_proj); SiLU(meta_proj)])",
        "               fused = g * img_proj + (1-g) * meta_proj  ->  512d",
        "  Classifier : FC(512->1024->512->256->2) + BN+SiLU+Dropout",
        ...
        "",
        "Difference from Fusion_Gated: img_se=SEBlock(1280,16) instead of None",
        "Difference from Img_SE:       fusion=GatedFusion(512d) instead of Concat(1536d)",
        "",
        sep,
